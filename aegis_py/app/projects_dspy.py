...
_SIGNATURES: Dict[str, Dict[str, Any]] = {
    ...
    "OutreachColdEmail": {
        "instructions": (
            "Ты пишешь ХОЛОДНОЕ B2B-письмо владельцу сайта от лица SEO-агентства. "
            "Пиши как живой человек эксперту, а не как робот-рассыльщик. Строго "
            "4 коротких абзаца (120–170 слов всего), каждый — законченная мысль, "
            "НИКОГДА не обрывай предложение. Структура: (1) зацепка — конкретный "
            "факт об ИХ сайте (домен/ниша/город), без воды и без «Здравствуйте, "
            "меня зовут»; (2) проблема В ЦИФРАХ — используй динамику видимости "
            "ДОСЛОВНО из данных и раздели тренды: при падении покажи потерю "
            "(«видимость в Google упала на 42%: с 810 до 469 запросов в топ-50»), "
            "затем проекцию падение→трафик→заявки→₽; при росте — где недобираете "
            "против конкурентов; (3) оффер — бесплатный видео-аудит с разбором "
            "минимум 3 точек роста, ни к чему не обязывает; (4) CTA — короткий "
            "живой вопрос. Тон деловой, тёплый, уверенный. ЗАПРЕЩЕНЫ канцелярит "
            "и клише: уникальный, эффективный, профессиональный, качественный, "
            "комплексный, инновационный, надёжный, широкий спектр, команда "
            "профессионалов, индивидуальный подход. Не выдумывай цифры — только "
            "из данных динамики. subject ≤ 55 символов, с цифрой потери и/или "
            "доменом, без спам-слов (бесплатно/скидка/срочно/акция)."
        ),
        "demos": [
            "зацепка: «Заметил ваш сайт remont-spb.ru по ремонту квартир в СПб — "
            "и сразу увидел, что позиции просели» (конкретно, без воды)",
            "проблема в цифрах: «За полгода видимость в Яндексе упала на 38%: "
            "с 540 до 335 запросов в топ-50. Для ремонта в СПб это примерно "
            "15–25 недополученных заявок в месяц»",
            "оффер+CTA: «Запишу короткое видео с разбором 3 точек роста именно "
            "по вашему сайту — бесплатно. Прислать?»",
        ],
    },
}
...
if _DSPY_AVAILABLE:  # pragma: no cover - требует установленного dspy-ai
    ...
    class OutreachColdEmail(dspy.Signature):  # type: ignore
        """«Человеческое» холодное B2B-письмо владельцу сайта от SEO-агентства."""

        niche = dspy.InputField(desc="Ниша сайта-получателя (например, имплантация)")
        city = dspy.InputField(desc="Город/регион сайта-получателя")
        dynamics = dspy.InputField(desc="Динамика видимости keys.so (deviation_pct, first/last, trend)")
        email = dspy.OutputField(desc="JSON {subject,html}: 4 абзаца 120–170 слов, тон эксперта, цифры из dynamics")
    ...
    _DSPY_SIGNATURE_CLASSES = {
        ...
        "OutreachColdEmail": OutreachColdEmail,
    }
else:
    _DSPY_SIGNATURE_CLASSES = {}
