...
.table-wrap {
  ...
  border-radius: 6px;
  /* Промоутим скролл-контейнер в собственный композитный слой. Без этого
   * Chromium у части пользователей НЕ перерисовывал таблицу, вставленную
   * асинхронно через v-if/v-for (карточка `.card` появляется после загрузки
   * `status`/`runs`/…): содержимое внутри `overflow-x: auto` оставалось
   * «непрорисованным» до первого принудительного repaint — а единственным
   * его триггером был `.card:hover { transform }`. Отсюда баг «таблицы
   * появляются только при наведении». Отдельный слой делает отрисовку
   * независимой от hover родителя и чинит это окончательно. */
