...
import ForecasterPrefillV2 from '../components/ForecasterPrefillV2.vue';
...
// ДОПОЛНЕНИЕ (V2): раскладываем результат компонента «Заполнить по сайту» по
// полям формы. Логику формы не заменяет — только заполняет поля. Вся логика
// сбора данных — внутри ForecasterPrefillV2.
function applyPrefill(data) {
  if (!data || data.ok === false) return;
  inputMode.value = 'keywords';
  const kws = data.source?.keywords || [];
  form.value.keywords_text = kws.join('\n');
  if (data.name && !form.value.name) form.value.name = data.name;
  const o = data.options || {};
  if (o.target_url) form.value.target_url = o.target_url;
  if (o.current_traffic_per_month != null) form.value.current_traffic_per_month = String(o.current_traffic_per_month);
  if (o.region) form.value.region = o.region;
  if (o.main_query) form.value.main_query = o.main_query;
  if (o.conversion_rate) form.value.conversion_rate_pct = String(Math.round(o.conversion_rate * 1000) / 10);
  if (o.intent) form.value.intent = o.intent;
}
    ...
    <div class="p-6 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <section class="bg-gray-900 border border-gray-800 rounded-xl p-5">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-3">
          <!-- ДОПОЛНЕНИЕ (V2): авто-заполнение по сайту, отдельный компонент -->
          <ForecasterPrefillV2 @apply="applyPrefill" />
