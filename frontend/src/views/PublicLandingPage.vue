...
const plans = [
  {
    ...
    description: 'Проверить гипотезы, собрать семантику и выпускать контент без лишней рутины.',
    limit: 'Пакет лимитов',
    features: ['Исследование тем и интентов', 'SEO-тексты по ТЗ', 'Базовые отчёты и история'],
    featured: false,
  },
  {
    name: 'Growth',
    kicker: 'Для системного роста',
    description: 'Единый pipeline для контент-команд, которым важны скорость, качество и измеримый результат.',
    limit: 'Расширенный пакет',
    features: ['Все AI-инструменты платформы', 'E-E-A-T, LSI и quality gates', 'Проекты, отчёты и аналитика'],
    featured: true,
  },
  {
    name: 'Agency',
    kicker: 'Для агентств и команд',
    description: 'Гибкая ёмкость лимитов для нескольких проектов и прозрачной работы с клиентами.',
    limit: 'Индивидуальный объём',
    features: ['Несколько рабочих пространств', 'Премиальные отчёты для клиента', 'Масштабирование под нагрузку'],
    featured: false,
  },
];
...
const faqs = [
  {
    ...
    answer: 'Лимиты — это понятный ресурс для работы AI-инструментов: исследования, анализа, генерации контента и подготовки отчётов. Пакет подбирается под объём задач и количество проектов.',
  },
  {
...
onMounted(() => {
  ...
  schemaScript.textContent = JSON.stringify({
    ...
    offers: { '@type': 'Offer', category: 'AI SEO platform', availability: 'https://schema.org/OnlineOnly' },
    featureList: ['SEO content generation', 'Search intent research', 'E-E-A-T quality control', 'SEO reports'],
  });
...
<template>
  <main class="marketing-landing">
      ...
      <div class="landing-container hero-grid">
        <div class="hero-copy">
          ...
          <div class="hero-actions">
            <RouterLink to="/register" class="button button-primary">Попробовать бесплатно <span>↗</span></RouterLink>
            <a href="#platform" class="button button-ghost">Посмотреть платформу <span class="play-icon">▶</span></a>
          </div>
          <div class="hero-note"><span class="note-check">✓</span> Без банковской карты на старте <span class="note-separator">·</span> Русский язык и кириллица native</div>
        </div>
    ...
    <section id="plans" class="plans-section section-pad">
      <div class="landing-container"><div class="section-heading plans-heading"><div><div class="eyebrow muted-eyebrow">Лимиты под ваш темп</div><h2>Начните с малого.<br /><span>Масштабируйте уверенно.</span></h2></div><p>Выбирайте объём под текущую задачу и подключайте больше мощности, когда растёт контентный план.</p></div><div class="plans-grid"><article v-for="plan in plans" :key="plan.name" class="plan-card" :class="{ 'plan-featured': plan.featured }"><div class="plan-top"><span class="plan-kicker">{{ plan.kicker }}</span><span v-if="plan.featured" class="popular-badge">Популярный</span></div><h3>{{ plan.name }}</h3><p>{{ plan.description }}</p><div class="plan-limit">{{ plan.limit }}</div><ul><li v-for="feature in plan.features" :key="feature"><span>✓</span>{{ feature }}</li></ul><RouterLink to="/register" class="plan-link" :class="{ 'plan-link-primary': plan.featured }">{{ plan.featured ? 'Начать с Growth' : 'Выбрать пакет' }} <span>↗</span></RouterLink></article></div><p class="plans-footnote">Точный объём лимитов и условия подключаются под ваш сценарий. Без скрытых этапов и непонятных ограничений.</p></div>
    </section>
...
.plans-section { background: var(--paper); color: var(--ink); }.plans-heading { margin-bottom: 48px; }.plans-grid { display: grid; grid-template-columns: repeat(3,1fr); gap: 14px; }.plan-card { position: relative; display: flex; flex-direction: column; min-height: 385px; padding: 29px 28px 25px; border: 1px solid var(--line-light); border-radius: 7px; background: #ebe9e5; }.plan-card.plan-featured { color: #f4f2f2; border-color: #28243e; background: #211e36; box-shadow: 0 20px 45px rgba(24,20,52,.17); transform: translateY(-11px); }.plan-top { display: flex; justify-content: space-between; align-items: center; min-height: 18px; }.plan-kicker { color: #7d7b81; font-size: 9px; letter-spacing: .13em; text-transform: uppercase; }.plan-featured .plan-kicker { color: #aaa1ef; }.popular-badge { padding: 5px 7px; border-radius: 3px; background: rgba(168,240,214,.11); color: var(--mint); font-size: 8px; font-weight: 700; }.plan-card h3 { margin: 33px 0 11px; font-size: 34px; letter-spacing: -.05em; }.plan-card p { max-width: 275px; min-height: 58px; margin: 0; color: #77767c; font-size: 12px; line-height: 1.6; }.plan-featured p { color: #9995aa; }.plan-limit { margin: 24px 0 16px; color: #554bbd; font-size: 12px; font-weight: 800; }.plan-featured .plan-limit { color: var(--mint); }.plan-card ul { display: grid; gap: 10px; margin: 0; padding: 17px 0 21px; border-top: 1px solid var(--line-light); list-style: none; }.plan-featured ul { border-color: rgba(255,255,255,.13); }.plan-card li { display: flex; gap: 8px; color: #6d6b73; font-size: 11px; }.plan-featured li { color: #c0bdca; }.plan-card li span { color: #6a60ce; }.plan-featured li span { color: var(--mint); }.plan-link { display: flex; justify-content: space-between; align-items: center; margin-top: auto; padding-top: 17px; border-top: 1px solid var(--line-light); color: #5a50bc; font-size: 11px; font-weight: 800; text-decoration: none; }.plan-featured .plan-link { border-color: rgba(255,255,255,.13); color: var(--mint); }.plan-link span { font-size: 17px; }.plans-footnote { margin: 25px 0 0; color: #85848a; font-size: 10px; text-align: center; }
