...
async function createInfoArticleTask(req, res, next) {
  try {
    ...
    const publishedUrlRaw = body.published_url ? sanitizeUrl(clipStr(body.published_url, 500)) : null;
    const publishedUrl = publishedUrlRaw && /^https?:\/\//i.test(publishedUrlRaw) ? publishedUrlRaw : null;
    ...
    return res.status(201).json({
      task: isClientRequest(req) ? sanitizeTaskForClient(task) : task,
