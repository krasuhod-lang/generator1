...
const FORECASTER_CONFIG = deepFreeze({
  ...
  // ── «Ванга» — лаконичное бизнес-саммари прогноза (Gemini) ─────────
  // Отдельный короткий вызов: человеческое резюме для владельца бизнеса
  // («что меня ждёт»). Cost control: жёсткий потолок вывода — maxChars
  // символов (продублирован в системном промпте и серверной обрезкой).
  // При любом сбое Gemini (429/500/timeout) — verdict 'skipped'/'error',
  // пайплайн НЕ прерывается; фронт показывает плейсхолдер.
  vanga: {
    enabled:     true,
    temperature: 0.4,
    maxTokens:   400,      // ~800 символов русского текста с запасом
    timeoutMs:   45000,
    maxChars:    800,      // жёсткая серверная обрезка текста саммари
