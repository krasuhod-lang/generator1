...
const FORECASTER_CONFIG = deepFreeze({
  ...
  unified: {
    ...
    // Ramp-up старта: логистика при t=1 и t0=6 уже даёт ~15 % пути к потолку,
    // и на большом ядре (L0 в сотни тысяч) это превращается в «прыжок»
    // трафика в первый же месяц. rampUpMonths — за сколько месяцев кривая
    // захвата «отпускается» на полную: t=1 → 1/4, t=2 → 2/4, …, t ≥ 4 → 1.0.
    rampUpMonths: 4,

    // Жёсткий cap прироста трафика относительно старта (защита от скачка
    // «со 100 до 4500 визитов» в M1). Применяется только когда пользователь
    // ввёл текущий трафик (curTraffic > 0):
    //   cap(t) = max(curTraffic·(1 + growthCapPerMonth·t),
    //                curTraffic + absGrowthPerMonth·t)
    // Первый множитель ограничивает крупные сайты (t=1 → ×1.4, t=2 → ×1.8),
    // второй даёт маленьким сайтам расти в абсолюте (+300 визитов/мес).
    growthCapPerMonth: 0.4,
    absGrowthPerMonth: 300,
    ...
    // impressionCtr — среднестатистический CTR «показ → визит»: доля
    // показов сайта в выдаче, которая превращается в переход. Отсюда
    // объём показов = визиты / impressionCtr (ограничен объёмом спроса).
    // Значение усреднённое по коммерческой выдаче RU (видимость топ-5..10).
    impressionCtr:    0.22,
    impressionCtrMin: 0.05,
    impressionCtrMax: 0.60,
  },
  ...
  // Отдельный вызов: человеческое клиентское резюме («что меня ждёт»),
  // теперь с опорой на перечни работ прогнозатора. Cost control: потолок
  // вывода — maxChars символов (продублирован в системном промпте и
  // серверной обрезкой). При любом сбое Gemini (429/500/timeout) — verdict
  // 'skipped'/'error', пайплайн НЕ прерывается; фронт показывает плейсхолдер.
  vanga: {
    enabled:     true,
    temperature: 0.4,
    maxTokens:   900,      // ~1800 символов русского текста с запасом
    timeoutMs:   45000,
    maxChars:    1800,     // жёсткая серверная обрезка текста саммари
    maxWords:    320,      // лимит из системного промпта (для модели)
  },
  ...
  keywordsFilter: {
    ...
    // если после фильтра осталось МЕНЬШЕ этого числа фраз — задача
    // завершается с кодом failed_no_commercial_intent (вместо деления на
    // ноль в математике дальше по пайплайну).
    minCommercialPhrases: 1,
  },
