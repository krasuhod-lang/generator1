...
const MAX_SEMANTIC_CLAIMS = 30;   // LLM-аудит текстовых фактов батчем

// Fail-mode политики семантического фактчекинга (Итерация 2, Задача 3):
//   open (default) | closed_ymyl | closed_all. Полная логика (расписание
...
// Текстовые факт-сигналы без чисел: определения, события, имена собственные.
const SEMANTIC_DEFINITION_RE = /(?:[—-]\s*это|\bявля(?:ет|ют)ся\b|\bназыва(?:ет|ют)ся\b|\bпредставля(?:ет|ют) собой\b|\bотнос(?:ит|ят)ся к\b)/i;
const SEMANTIC_EVENT_VERB_RE = /\b(?:основан[аоы]?|создан[аоы]?|запущен[аоы]?|открыт[аоы]?|закрыт[аоы]?|объявил[аи]?|подписал[аи]?|вступил[аи]?|победил[аи]?|получил[аи]?|разработал[аи]?|изобр[её]л|выпустил[аи]?|принадлежит|расположен[аоы]?|состоит|включает|признан[аоы]?)\b/i;
const SEMANTIC_WORD_DATE_RE = /\b(?:январ[ьяе]|феврал[ьяе]|март[ае]?|апрел[ьяе]|ма[йяе]|июн[ьяе]|июл[ьяе]|август[ае]?|сентябр[ьяе]|октябр[ьяе]|ноябр[ьяе]|декабр[ьяе]|понедельник|вторник|сред[ауе]|четверг|пятниц[ауе]|суббот[ауе]|воскресень[ея]|весн[аойе]|летом|осень[юьи]?|зим[аойе])\b/i;
const SEMANTIC_PROPER_NOUN_RE = /[а-яёa-z,;:()«»"'\-]\s+([A-ZА-ЯЁ][a-zа-яё]+(?:\s+[A-ZА-ЯЁ][a-zа-яё]+){0,3})/;
...
/**
 * extractSemanticClaims(textOrHtml) → текстовые фактологические утверждения
 * БЕЗ числовых токенов: прежний deterministic fact-check их не видел, поэтому
 * отдаём их отдельному LLM-слою с жёсткой сверкой по SERP evidence.
 */
function extractSemanticClaims(textOrHtml) {
  const plain = stripHtml(textOrHtml);
  const sentences = splitSentences(plain);
  const seen = new Set();
  const claims = [];
  for (const raw of sentences) {
    if (claims.length >= MAX_SEMANTIC_CLAIMS) break;
    const sentence = raw.trim();
    if (sentence.length < MIN_CLAIM_CHARS) continue;
    if (sentence.length > MAX_CLAIM_CHARS) continue;
    if (extractFactTokens(sentence).length > 0) continue;

    const hasDefinition = SEMANTIC_DEFINITION_RE.test(sentence);
    const hasEvent = SEMANTIC_EVENT_VERB_RE.test(sentence);
    const hasWordDate = SEMANTIC_WORD_DATE_RE.test(sentence);
    const hasProperNoun = SEMANTIC_PROPER_NOUN_RE.test(sentence);
    if (!hasDefinition && !hasEvent && !hasWordDate && !hasProperNoun) continue;
    ...
    claims.push({
      ...
      kinds: [
        ...
        ...(hasEvent ? ['event'] : []),
        ...(hasWordDate ? ['word_date'] : []),
        ...(hasProperNoun ? ['proper_noun'] : []),
      ],
    });
  }
  return claims;
...
function _semanticSkipped(claims, reason) {
  const arr = (claims || []).map((c) => ({
    id: c.id,
    text: c.text,
    kinds: c.kinds || ['semantic'],
    status: 'skipped',
    source_url: null,
    reason,
  }));
  arr.semanticSkipped = true;
  arr.reason = reason;
  return arr;
}

function _evidenceForSemanticPrompt(evidenceResult, maxSnippets = 40) {
  const evItems = (evidenceResult && Array.isArray(evidenceResult.evidence))
    ? evidenceResult.evidence : [];
  const snippets = [];
  for (const it of evItems) {
    const url = it.url || '';
    const h1 = it.h1 || '';
    for (const sn of (Array.isArray(it.snippets) ? it.snippets : [])) {
      const text = String((sn && sn.text) || '').replace(/\s+/g, ' ').trim();
      if (!text) continue;
      snippets.push({ url, h1, text: text.slice(0, 700) });
...
function _parseSemanticJson(text) {
  ...
  if (!Array.isArray(parsed)) throw new Error('semantic verifier returned non-array JSON');
  return parsed;
}

function _normalizeSemanticStatus(status) {
...
/**
 * verifySemanticClaims — LLM-сверка текстовых фактов с SERP evidence.
 * Fail-open: отсутствие ключа, ошибка вызова или парсинга возвращают skipped.
 */
async function verifySemanticClaims(claims, evidenceResult, opts = {}) {
  const list = Array.isArray(claims) ? claims.slice(0, MAX_SEMANTIC_CLAIMS) : [];
  if (!list.length) {
    const empty = [];
  ...
  try {
    ...
    const evidence = _evidenceForSemanticPrompt(evidenceResult, opts.maxEvidenceSnippets || 40);
    const systemInstruction = [
      'Ты — строгий фактчекер. Проверяй ТОЛЬКО переданные утверждения против SERP evidence snippets.',
      'Классифицируй каждое утверждение как supported, partial или unsupported.',
      'supported разрешён только при поддержке Tier-1/авторитетного источника из evidence; для supported обязательно укажи source_url.',
      'Не используй внешние знания и не додумывай факты. Если evidence слабый, ставь partial или unsupported.',
      'Верни ТОЛЬКО строгий JSON-массив: [{"id":1,"status":"supported|partial|unsupported","source_url":"https://... или null"}].',
    ].join('\n');
    ...
    const out = list.map((c) => {
      ...
      const status = _normalizeSemanticStatus(r.status);
      return {
        id: c.id,
        text: c.text,
        kinds: c.kinds || ['semantic'],
        status,
        source_url: status === 'supported' && r.source_url ? String(r.source_url) : null,
      };
    });
...
function summarizeSemantic(results) {
  const list = Array.isArray(results) ? results : [];
  const summary = { total: list.length, supported: 0, partial: 0, unsupported: 0, skipped: 0 };
  for (const r of list) {
    if (r.status === 'supported') summary.supported += 1;
...
function _summaryWithSemantic(base, semanticSummary, semanticRan) {
  if (!semanticRan) return base;
  const total = (base.total || 0)
    + (semanticSummary.supported || 0)
    + (semanticSummary.partial || 0)
    + (semanticSummary.unsupported || 0);
  const supported = (base.supported || 0) + (semanticSummary.supported || 0);
  const partial = (base.partial || 0) + (semanticSummary.partial || 0);
  const unsupported = (base.unsupported || 0) + (semanticSummary.unsupported || 0);
  const supportedPct = total === 0 ? 0 : Math.round((supported / total) * 1000) / 10;

  let verdict = 'fail';
  if (total === 0) verdict = 'na';
  else if (supported / total >= PASS_THRESHOLD && unsupported <= MAX_UNSUPPORTED_FOR_PASS) verdict = 'pass';
  else if (supported / total >= REVIEW_THRESHOLD) verdict = 'review';
...
/**
 * _annotateSemanticFailMode — обогатить semantic-блок отчёта информацией о
 * fail-mode политике (Задача 3). Не принимает решение о блокировке (это делает
 * quality gate checker), но фиксирует контекст: режим, YMYL-признак ниши,
 * недоступность семантики и итоговый флаг failClosed для UI/логов/аудита.
 * @param {object} semantic — блок { semanticSkipped, reason, ... }
 * @param {object} opts — { niche, failMode }
 * @returns {object} тот же semantic с добавленными полями
 */
function _annotateSemanticFailMode(semantic, opts = {}) {
  const failMode = semanticFactcheckPolicy.resolveFailMode(opts.failMode);
  const isYmyl = semanticFactcheckPolicy.isYmylNiche(opts.niche || '');
  const unavailable = semantic.semanticSkipped === true;
  const failClosed = unavailable && semanticFactcheckPolicy.shouldFailClosed({ failMode, isYmyl });
...
/**
 * runSemanticFactCheck — deterministic numeric layer + LLM semantic layer.
 * Сигнатура deterministic runFactCheck не меняется; новый фасад возвращает
 * superset и сохраняет совместимость с fact_check_report.
 *
 * opts.niche / opts.failMode — контекст fail-mode политики (Задача 3):
 * прокидываются в semantic-блок отчёта, чтобы quality gate мог выставить
 * blocker semantic_factcheck_unavailable для YMYL-ниш.
 */
async function runSemanticFactCheck(html, evidenceResult, opts = {}) {
  const deterministic = runFactCheck(html, evidenceResult, opts);
  try {
    const semanticClaims = extractSemanticClaims(html);
    const semanticResults = await verifySemanticClaims(semanticClaims, evidenceResult, opts.semantic || opts);
    const semanticSummary = summarizeSemantic(semanticResults);
    const semanticSkipped = semanticResults.semanticSkipped === true;
    const semanticRan = !semanticSkipped && semanticSummary.total > 0;
    return {
      ...deterministic,
      summary: _summaryWithSemantic(deterministic.summary, semanticSummary, semanticRan),
      semantic: _annotateSemanticFailMode({
        claims: semanticResults,
        summary: semanticSummary,
        semanticSkipped,
        reason: semanticResults.reason || null,
      }, opts),
    };
  } catch (err) {
    const semanticClaims = extractSemanticClaims(html);
    const skipped = _semanticSkipped(semanticClaims, err && err.message ? err.message : String(err));
    return {
      ...deterministic,
      semantic: _annotateSemanticFailMode({
        claims: skipped,
        summary: summarizeSemantic(skipped),
...
module.exports = {
  ...
  _matchToken,
  _tokensInText,
  _annotateSemanticFailMode,
  // Fail-mode политика (Задача 3)
  semanticFactcheckPolicy,
  // Constants
  MIN_CLAIM_CHARS,
  MAX_CLAIM_CHARS,
  MAX_CLAIMS,
