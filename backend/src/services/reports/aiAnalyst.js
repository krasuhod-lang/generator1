...
Сделай 3–5 объектов в growth_attribution — по одному на ключевую метрику (Google clicks, Yandex clicks, видимость Keys.so, ТОП-10/ТОП-3, объём задач и т.п.). Для quick_wins используй запросы на позициях 11-15. Для roadmap дай 5-7 коротких пунктов на следующий месяц. Если данных по метрике нет — не выдумывай, просто пропусти её.`;
...
async function generateSummary(data, opts = {}) {
  ...
  const userPrompt = [
    ...
    '',
    'Сформируй executive_summary (3 абзаца), 3-5 highlights, growth_attribution ' +
    '(массив из 3–5 объектов по ключевым метрикам с полями metric, attribution, ' +
    'conclusion, forecast, weak_zones), quick_wins, vulnerabilities, roadmap и traffic_value. ' +
    'Используй реальные числа из метрик и ссылайся на конкретные выполненные работы. Ответь СТРОГО JSON-объектом, ' +
