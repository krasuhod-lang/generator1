...
// математически (из итогов графиков), ИИ их не выдумывает.
...
function _canonicalNumbers(digest) {
  const map = {};
  const put = (id, last, prev, pct, unit) => {
    map[id] = {
      trend_direction: _trendDir(pct),
      delta_value: (last != null && prev != null) ? _deltaStr(last, prev, unit) : '',
      delta_pct: _pctStr(pct),
    };
  };
  put('gsc_clicks', digest.gsc_clicks_last, digest.gsc_clicks_prev, digest.gsc_clicks_delta_pct, 'кликов');
  put('gsc_impressions', digest.gsc_impressions_last, digest.gsc_impressions_prev, digest.gsc_impressions_delta_pct, 'показов');
  put('ywm_clicks', digest.ywm_clicks_last, digest.ywm_clicks_prev, digest.ywm_clicks_delta_pct, 'кликов');
  put('ywm_impressions', digest.ywm_impressions_last, digest.ywm_impressions_prev, digest.ywm_impressions_delta_pct, 'показов');
  put('keys_so_visibility', null, null, digest.keys_so_visibility_delta_pct, '');
  put('keys_so_google_visibility', null, null, digest.keys_so_google_visibility_delta_pct, '');
  put('keys_so_top10', digest.keys_so_top10_last, digest.keys_so_top10_prev, digest.keys_so_top10_delta_pct, 'запросов');
  put('keys_so_google_top10', digest.keys_so_google_top10_last, digest.keys_so_google_top10_prev, digest.keys_so_google_top10_delta_pct, 'запросов');
  return map;
}
