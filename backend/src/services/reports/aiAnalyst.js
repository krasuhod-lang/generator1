...
5. Обязательно выделяй **жирным шрифтом** (двойными звёздочками markdown) ключевые тезисы и цифры — главные достижения, проценты роста и абсолютные значения, чтобы они сразу бросались в глаза при чтении.
...
Ожидаемый формат ответа:
{
  ...
  "growth_attribution": [
    {
      ...
      "attribution": "Что именно из выполненных работ повлияло на рост этой метрики — привяжи к конкретным задачам (1–2 предложения). Если метрика снизилась — предложи гипотезу причины (сезонность спроса, обновление алгоритмов Яндекса/Google, давление конкурентов) и укажи, что это временное явление.",
  ...
  "next_month_forecast": "string (1-2 предложения — конкретный прогноз роста в процентах или кликах на следующий месяц на основе текущей динамики)",
...
- Если метрика выросла — начинай attribution со слова «Рост» и указывай конкретные цифры, а также чётко указывай, ЗА СЧЁТ ЧЕГО произошёл рост (новые страницы, CTR, позиции).
...
async function generateSummary(data, opts = {}) {
  ...
  const pass3Prompt = [
    ...
    '{',
    '  "executive_summary": "3 абзаца: 1) главное достижение с числами, 2) факторы роста и связь с работами, 3) прогноз. Выделяй ключевые тезисы и цифры **жирным** (markdown-звёздочками)",',
    '  "highlights": ["3-5 ярких пунктов, каждый начинается с цифры роста"],',
    '  "quick_wins": [{query, position, plan}],',
    '  "next_month_forecast": "1-2 предложения — конкретный прогноз роста в процентах или кликах на следующий месяц на основе текущей динамики",',
