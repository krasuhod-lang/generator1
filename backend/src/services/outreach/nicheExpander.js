...
const SYSTEM_PROMPT = `Ты SEO-аналитик. Отвечай ТОЛЬКО валидным JSON, без markdown, без пояснений.
По поисковому запросу определи нишу и дай 3 коммерческих запроса для поиска сайтов компаний.
Формат ответа (строго):
{"niche":"название ниши","business_type":"B2C","queries":["запрос 1","запрос 2","запрос 3"],"niche_description":"описание"}`;

async function analyzeNiche(keyword) {
  const result = await callLLM('deepseek', SYSTEM_PROMPT, keyword, {
    retries: 3, temperature: 0.1, maxTokens: 1000,
