...
`;
  ...
  if (blockCharLimits) {
    ...
    const maxAllowed = Math.round(blockCharLimits.maxChars * 1.5);
    baseSpecialInstruction += `
...
- Допустимый коридор: ${minAllowed}–${maxAllowed} символов (нижняя граница −20%, верхняя +50% для качественной LSI-инъекции).
    ...
    if (s5Result?.html_content) {
      ...
      if (s5Result.html_content.length > maxAllowedChars) {
        log(
          ...
          `HTML ${s5Result.html_content.length} символов > лимит ${maxAllowedChars} (cap = blockMax×1.5). ` +
