...
const SYSTEM_PROMPT = [
  ...
  '',
  'ВАЖНО: Опиши динамику КАЖДОГО ключевого графика отдельно:',
  '  • **Клики**: растут/падают/стабильны — за счёт чего (какие запросы/',
  '    страницы двигают, сезонность, изменения в выдаче). Если падение —',
  '    назови конкретную причину и аргументируй (потеря позиций по каким',
  '    запросам, снижение спроса в нише, действия конкурентов, технические',
  '    проблемы). Если рост — объясни, за счёт чего именно (новый контент,',
  '    улучшение позиций, рост спроса по категории).',
  '  • **Показы**: тренд и причины изменений (расширение/сужение',
  '    семантического охвата, сезонный спрос, индексация новых страниц).',
  '  • **CTR**: динамика и факторы (улучшение сниппетов, изменение позиций,',
  '    конкуренция за внимание в SERP, появление/исчезновение rich results).',
  '  • **Средняя позиция**: тренд, какие запросы/группы тянут вверх или вниз.',
  '',
  'Для каждого графика дай СИТУАЦИОННУЮ ОЦЕНКУ:',
  '  — если падение: «Ситуация тревожная / требует внимания, потому что…»',
  '  — если рост: «Положительная динамика за счёт…, для закрепления нужно…»',
  '  — если стабильность: «Метрика стабильна, потенциал роста в…»',
  '',
  '## 2. Точки роста',
  ...
  '',
    ...
    'рекомендаций на размещение ссылок в формате таблицы: «рекомендуемый анкор |',
    'варианты анкоров | тип | формат и тема статьи донора | целевой URL | приоритет |',
    'competition signal | evidence | зачем». Используй anchor_plan.recommended_anchor',
    'и варианты, а не повторяй exact-match механически. Если anchor_plan.risk=diversify,',
    'обязательно предложи разбавление и не ставь exact-match подряд.',
    'Для каждой темы используй article_brief.format/angle/evidence/link_rule: статья',
    'должна закрывать читательский вопрос и иметь самостоятельную пользу, а ссылка —',
    'быть частью контекста, а не единственной целью публикации.',
    'Отдельно оцени доноров (каких стоит рассматривать, каких избегать) и анкор-профиль',
    '(перекосы, переоптимизация). Не выдумывай backlink-метрики конкурентов: если',
    'competitive_basis.competitor_backlink_data=false, прямо укажи, что это рекомендации',
    'по собственным GSC/SERP/content-сигналам, а конкурентный backlink gap требует отдельного источника.',
  '',
  ...
  '«тема | H1 | title (70-80) | description (180-190) | целевой интент URL |',
  ...
  '',
  '## 15. Выводы и прогноз',
  'ОБЯЗАТЕЛЬНЫЙ финальный раздел. На основе ВСЕХ проанализированных данных:',
  '',
  '### Ключевые выводы',
  'Кратко (3-5 пунктов) — что ГЛАВНОЕ показал анализ, какие проблемы самые',
  'срочные и какие возможности самые перспективные.',
  '',
  '### Прогноз на ближайшие 1-3 месяца',
  'Аргументированный прогноз динамики каждой ключевой метрики:',
  '  • **Клики**: ожидаемый тренд и за счёт чего (конкретные запросы/страницы).',
  '  • **Показы**: как изменится охват (новый контент, сезонность, индексация).',
  '  • **CTR**: потенциал улучшения (сниппеты, позиции, микроразметка).',
  '  • **Позиции**: по каким группам запросов ожидается движение.',
  '',
  '### Потенциал роста при выполнении плана',
  'Оценка ожидаемого эффекта при реализации рекомендованных действий.',
  'Какие метрики вырастут первыми, где эффект будет заметен быстрее, какие',
  'риски могут помешать (сезонность, конкуренция, технический долг).',
  'Не выдумывай цифр — давай качественную, аргументированную оценку.',
  '',
...
function _renderLinkStrategyLines(link) {
  ...
  const out = ['', '[ССЫЛОЧНАЯ СТРАТЕГИЯ] (раздел 8 — выдай ≥5 рекомендаций)',
    ...
    `Рекомендации на размещение ссылок (anchor, recommended_anchor, anchor_variants, anchor_type, donor_topic — готовая тема статьи-донора напрямую (рабочий заголовок статьи под анкор, без служебной обёртки), article_brief, competition, donor_topic_ready/donor_topic_angle — проработанная тема и угол, donor_topic_title — интригующий SEO-title (раскрывает интент, не дублирует тему), donor_topic_description — meta-description, target_url, priority, why):`,
    JSON.stringify((link.recommendations || []).slice(0, 20).map((r) => ({
      anchor: r.anchor,
      recommended_anchor: r.anchor_plan && r.anchor_plan.recommended_anchor,
      anchor_variants: r.anchor_plan && r.anchor_plan.variants,
      anchor_risk: r.anchor_plan && r.anchor_plan.risk,
      anchor_type: r.anchor_type,
      article_brief: r.article_brief || null,
    ...
    `Конкурентная доказательная база: ${JSON.stringify(link.competitive_basis || {})}`];
  if (link.audit && link.audit.available) {
    ...
    out.push('', '[АУДИТ ССЫЛОК] (раздел 9)',
      ...
      `Топ-доноры (host, links, target_pages, coverage_score, trust_score, flags): ${JSON.stringify((a.donors || []).slice(0, 15).map((d) => ({
        host: d.host,
        links: d.links,
      ...
      `Итоги Top Linking Sites (donors, incoming_link_pages, target_pages): ${JSON.stringify(a.donor_totals || {})}`,
