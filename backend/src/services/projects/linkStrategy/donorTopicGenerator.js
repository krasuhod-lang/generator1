...
function _fallbackReadyTopic(seed) {
  ...
  if (/\bанализ\s+сайта\b/iu.test(s)) {
    return 'Как провести анализ сайта и найти точки роста в SEO';
  }
  if (/\bсемантическ\w*\s+ядр\w*/iu.test(s)) {
    if (/\b(цен[аы]|стоимост\w*)\b/iu.test(s)) {
  ...
  if (/\b(цен[аы]|стоимост\w*|умерен\w*)\b/iu.test(s)) {
  ...
  if (/\bпродвижен\w*|seo-?продвижен\w*/iu.test(s)) {
