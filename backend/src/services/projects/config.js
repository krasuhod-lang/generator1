...
const PROJECTS_CONFIG = deepFreeze({
  ...
  // Фоновый прогон «Анализ проекта» (analysisRunner). Раньше у тяжёлых
  // обогащающих слоёв (мета-аудит, E-E-A-T, ссылки, блог-план, GEO/AEO,
  // план действий) не было общего дедлайна: один зависший внешний запрос
  // держал анализ в статусе `running` бесконечно, и фронт вечно показывал
  // «ИИ анализирует…». Теперь каждый слой ограничен по времени, а зависшие
  // строки подчищает watchdog.
  analysisRun: {
    // Потолок на ОДИН обогащающий слой. Превышение → слой = null (graceful),
    // анализ продолжается на остальных данных.
    ...
    // Как часто watchdog проверяет зависшие строки.
  ...
  pageMetaAudit: {
    ...
    // Повторы staged-генерации мета-тега для одной страницы. Сбои SERP/LLM
    // транзиентны, а «там, где ошибка, генерация обязана срабатывать» — поэтому
    // повторяем с линейным backoff (delayMs × номер попытки).
    regenerateRetry: {
      attempts: 3,
      delayMs: 1500,
    },
