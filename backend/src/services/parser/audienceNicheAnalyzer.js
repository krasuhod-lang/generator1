...
1. AUDIENCE_PERSONAS (РОВНО 3 ПЕРСОНЫ): Учитывай расслоение аудитории — сегменты должны быть РАЗНЫМИ: массовый клиент, премиальный/делегирующий и скептически настроенный или чувствительный сегмент. Не создавай больше трёх персон.
   ...
   • pains                 — РОВНО 3 конкретные боли/страха. Без банальностей; описывай короткий сценарий с ситуацией, но не выдумывай факты бренда.
   • dark_fears            — 2 коротких внутренних страха персоны.
   • objections            — РОВНО 3 коротких возражения в формате прямых цитат: ценовое, скептическое и страховое. Формат: { "type": "ценовое|скептическое|страховое", "quote": "[короткая цитата]" }.
   • barriers              — 2 коротких внутренних/внешних препятствия, каждый как цитата-отговорка.
   • decision_criteria     — 3 конкретных критерия выбора, не общие «качество и цена».
   • content_triggers      — 3 формата или доказательных триггера, которые резонируют с персоной.
   • voice_examples        — РОВНО 3 короткие разговорные фразы в стиле реального поиска/форума; не используй маркетинговый язык и не добавляй пояснения.
...
2. NICHE_DEEP_DIVE (РОВНО 4 ИНСАЙТА): Каждый инсайт коротко показывает отличие ниши и даёт конкретное правило для копирайтера. Не создавай больше четырёх инсайтов.
   ...
   • insight               — 1–2 коротких предложения о неочевидном факторе ниши. Цифры допустимы только как общерыночные ориентиры и без выдуманных фактов бренда.
...
3. NICHE_TERMINOLOGY (8–10 LSI-сущностей): Короткие профессиональные термины/аббревиатуры с краткой расшифровкой. Не добавляй маркетинговые синонимы.
...
4. NICHE_RED_FLAGS (4–5 запрещённых формулировок): Коротко укажи опасную формулировку, причину и честную замену.
   Формат: массив объектов { "claim": "что нельзя обещать/писать", "reason": "почему", "replacement": "чем заменить" }. Для YMYL — минимум 4 пункта.
...
[SELF-AUDIT — silent, do not output]
       ✓ РОВНО 3 непохожие персоны?
       ✓ Все обязательные поля заполнены коротко?
       ✓ Нет выдуманных фактов бренда, цен, кейсов и гарантий?
       ✓ Для YMYL-ниши red flags и forbidden_voice содержательны?
       ✓ Ответ помещается в компактный JSON без markdown?

ВЕРНИ ТОЛЬКО JSON. НИКАКОГО ТЕКСТА ДО ИЛИ ПОСЛЕ.`;

/**
 * buildFallbackAnalysis — минимальный evidence-safe контекст при недоступности LLM.
 */
function buildFallbackAnalysis(task = {}) {
  const service = String(task.input_target_service || 'указанной услуги').trim();
  const audience = String(task.input_target_audience || 'целевой аудитории').trim();
  const businessType = String(task.input_business_type || 'указанной ниши').trim();
  const region = String(task.input_region || 'Россия').trim();
  return {
    analysis_status: 'fallback',
    audience_personas: [{
      name: 'Основной сегмент аудитории',
      demographics: `${audience}; регион: ${region}`,
      trigger_event: `Появилась задача найти решение по теме «${service}».`,
      buyer_journey_stage: 'consideration',
      jobs_to_be_done: `Когда возникает задача «${service}», клиент хочет понять варианты и выбрать подходящее решение в рамках своей ситуации.`,
      pains: [],
      dark_fears: [],
      objections: [],
      barriers: [],
      decision_criteria: ['понятные условия', 'подтверждённая информация', 'прозрачный процесс'],
      content_triggers: ['чеклист', 'таблица сравнения', 'разбор процесса'],
      voice_examples: [`как выбрать ${service}`, `что важно знать про ${service}`, `сколько времени занимает ${service}`],
      ideal_outcome: `Получить понятный план выбора решения по теме «${service}».`,
    }],
    niche_deep_dive: [{
      theme: 'Проверяемость',
      insight: `Для ниши «${businessType}» важно отделять подтверждённые сведения от предположений.`,
      content_implication: 'Показывать источники, условия применимости и ограничения каждого утверждения.',
    }],
    niche_terminology: [],
    niche_red_flags: [],
    content_voice: {
      tone: 'деловой-сдержанный',
      emotional_register: 'доверие через доказательства',
      sensory_focus: false,
      forbidden_voice: ['неподтверждённые гарантии', 'агрессивные обещания'],
      cta_style: 'предложить сравнить варианты и уточнить условия',
    },
  };
}
...
async function analyzeAudienceAndNiche(task, ctx, extra = {}) {
  ...
  try {
    const result = await callLLM(
      'deepseek',
      'Ты — стратег контент-маркетинга. Возвращай только компактный валидный JSON по схеме.',
      prompt,
      {
        // Компактный contract рассчитан на один provider-вызов; при редком
        // truncation разрешаем только один контролируемый повтор, а не
        // автоматическое удвоение до 32k, которое раздувает task budget.
        retries:     2,
        maxTokens:   12000,
    ...
    // brand facts, а в логах явно фиксируем, что нужен дополнительный audit.
    log(`Audience & Niche Analyzer: fallback после ошибки — ${err.message}`, 'warn');
    return buildFallbackAnalysis(task);
  }
}
