 ...
 *
 * Помимо императива распознаём инфинитивную форму («Сравнить условия»,
 * «Подобрать программу»): она частотна в реальных сниппетах, а её отсутствие
 * в словаре давало ложное «CTA отсутствует» (−15 к CTR-скору) и лишнюю
 * перегенерацию пары.
 */
...
const CTA_PARTS = [
  'узна(?:йте|й|ть)',
  'запи(?:ш(?:итесь|ись)|с(?:ывайтесь|аться|ь))',
  'подробн[а-яё]*',
  'закаж(?:ите|и|ем)',
  'заказыв[а-яё]*',
  'заказать',
  ...
  'смотрите',
  'посмотр(?:ите|еть)',
  // Сравнение / подбор / проверка — типовые CTA агрегаторов и подборок.
  'сравн(?:ите|и|ить)',
  'подбер(?:ите|и)',
  'подобрать',
  'заполн(?:ите|и|ить)',
...
const CTA_RE = new RegExp(`(?:${CTA_PARTS.join('|')})(?![а-яё])`, 'i');
