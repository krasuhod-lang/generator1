...
// без markdown и рассуждений, и держать значения краткими, чтобы уложиться в
// лимит вывода. Добавляется к user-промпту на второй+ попытке.
const JSON_REPAIR_HINT = `

[КРИТИЧНО — предыдущая попытка вернула ОБРЕЗАННЫЙ или НЕВАЛИДНЫЙ JSON]
Верни ТОЛЬКО валидный, полностью закрытый JSON строго по контракту:
- все фигурные/квадратные скобки и кавычки закрыты;
- без markdown-обёрток и без текста/рассуждений до или после JSON;
- значения полей держи КРАТКИМИ, чтобы гарантированно уложиться в лимит вывода;
- не добавляй полей вне контракта.`;

// Признак обрыва ответа лимитом токенов (Gemini finishReason=MAX_TOKENS,
// DeepSeek finish_reason=length) — по результату вызова или тексту ошибки.
function _isTruncation(res, err) {
  const fr = String((res && res.finishReason) || '').toUpperCase();
  if (fr === 'MAX_TOKENS' || fr === 'LENGTH') return true;
  const msg = String((err && err.message) || '').toLowerCase();
  return msg.includes('max_tokens') || msg.includes('truncat') || msg.includes('обрез');
}

// Потолок maxTokens (совпадает с валидацией адаптеров Gemini/DeepSeek).
 ...
 * Gemini fallback (нет ключа / ошибка вызова). Ретрай с эскалацией: при обрыве
 * ответа лимитом токенов повышаем maxTokens, при любом сбое парсинга добавляем
 * к промпту requirement «верни полный валидный JSON» и просим модель доработать.
 */
...
async function _callCopywriterJson(userPrompt, usage, opts = {}) {
  // gemini-3.1-pro-preview — reasoning-модель: thinking-токены тарифицируются
  // и ВХОДЯТ в maxOutputTokens, поэтому при слишком низком лимите JSON
  // обрывается на полуслове. Даём запас (8192) и эскалируем при обрыве.
  ...
  for (let attempt = 1; attempt <= MAX_PARSE_ATTEMPTS; attempt += 1) {
    let res = null;
    try {
      res = await callGemini(PAIR_ASSEMBLER_SYSTEM, prompt, {
