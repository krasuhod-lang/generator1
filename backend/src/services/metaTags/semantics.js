...
// Кириллический корпус стоп-слов из beta-версии + служебные слова, которые
// раньше просачивались в LSI («который», «также», «наш», «свой»).
const STOP_WORDS_RAW = [
  ...
  // Служебные слова, отсутствовавшие в beta-корпусе: без них TF-IDF отдавал
  // в LSI мусорные стеммы «котор», «такж», «нужн» и рекомендовал вписать их
  // в мета-теги.
  'который','которая','которое','которые','которых','которым','которой',
  'также','такие','такая','такое','свой','своя','свои','своего','своих',
  'наш','наша','наши','нашего','ваш','ваша','ваши','вашего',
  'весь','вся','всё','каждый','каждая','любой','любая','самый','самая',
  'нужно','нужен','нужна','этих','эта','этим','кроме','либо','всем','вами',
];

/**
 * Стоп-слова в НОРМАЛИЗОВАННОМ виде.
 *
 * Проверка идёт по стеммам (`STOP_WORDS.has(normalizeWord(w))`), поэтому сырой
 * корпус её проходил лишь частично: «можно» → `можн`, «лучше» → `лучш`,
 * «больше» → `больш` в множестве отсутствовали и попадали в LSI. Держим в Set
 * и сырые формы (для внешних потребителей), и их стеммы.
 */
const STOP_WORDS = new Set([
  ...STOP_WORDS_RAW,
  ...STOP_WORDS_RAW.map((w) => normalizeWord(w)),
].filter(Boolean));
