...
async function ensureDurableTaskSchema(db = dbDefault) {
  const statements = [
    ...
    `ALTER TABLE tasks ADD COLUMN IF NOT EXISTS content_stale BOOLEAN NOT NULL DEFAULT FALSE`,
    `ALTER TABLE info_article_tasks ADD COLUMN IF NOT EXISTS archived_at TIMESTAMPTZ`,
    `ALTER TABLE info_article_tasks ADD COLUMN IF NOT EXISTS archived_by UUID`,
    `ALTER TABLE info_article_tasks ADD COLUMN IF NOT EXISTS last_started_at TIMESTAMPTZ`,
    `ALTER TABLE link_article_tasks ADD COLUMN IF NOT EXISTS archived_at TIMESTAMPTZ`,
    `ALTER TABLE link_article_tasks ADD COLUMN IF NOT EXISTS archived_by UUID`,
    `ALTER TABLE link_article_tasks ADD COLUMN IF NOT EXISTS last_started_at TIMESTAMPTZ`,
    `UPDATE info_article_tasks SET last_started_at=started_at WHERE last_started_at IS NULL AND started_at IS NOT NULL`,
    `UPDATE link_article_tasks SET last_started_at=started_at WHERE last_started_at IS NULL AND started_at IS NOT NULL`,
    `UPDATE tasks SET last_started_at=started_at WHERE last_started_at IS NULL AND started_at IS NOT NULL`,
    `CREATE INDEX IF NOT EXISTS idx_tasks_user_last_started ON tasks(user_id, last_started_at DESC NULLS LAST, created_at DESC)`,
    ...
    // Commercial access foundation (migration 144). Additive/idempotent for old volumes.
    `CREATE TABLE IF NOT EXISTS user_access_profiles (
      user_id UUID PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
      account_role VARCHAR(20) NOT NULL DEFAULT 'client' CHECK (account_role IN ('admin','employee','client')),
      plan_key VARCHAR(20) NOT NULL DEFAULT 'trial' CHECK (plan_key IN ('trial','minimal','medium','pro','internal')),
    ...
    `CREATE INDEX IF NOT EXISTS idx_user_access_profiles_role ON user_access_profiles(account_role)`,
    `CREATE INDEX IF NOT EXISTS idx_user_access_profiles_active_period ON user_access_profiles(status, period_end)`,
    `INSERT INTO user_access_profiles (user_id, account_role, plan_key, status, period_start, period_end, overrides)
       SELECT id, CASE WHEN role='admin' THEN 'admin' ELSE 'client' END,
              CASE WHEN role='admin' THEN 'internal' ELSE 'trial' END,
              'active', COALESCE(created_at, NOW()), NULL, '{}'::jsonb
         FROM users u WHERE NOT EXISTS (SELECT 1 FROM user_access_profiles p WHERE p.user_id=u.id)`,
    `CREATE TABLE IF NOT EXISTS access_usage_reservations (
      ...
      state VARCHAR(20) NOT NULL DEFAULT 'reserved' CHECK (state IN ('reserved','consumed','released')),
    ...
    `CREATE INDEX IF NOT EXISTS idx_access_usage_period_resource ON access_usage_reservations(user_id, period_key, resource_key, state)`,
    `CREATE INDEX IF NOT EXISTS idx_access_usage_task ON access_usage_reservations(user_id, source, task_id)`,
