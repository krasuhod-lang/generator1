...
OUTPUT FORMAT: Верни СТРОГО валидный JSON без markdown-обёрток:
{
  ...
  // Подбор тем статей (article_topic_tasks, mode='topic_ideas') — Perplexity
  // sonar-pro как «Агент-Исследователь интентов». В отличие от
  // perplexityResearcher (свежие факты/цифры) этот промпт собирает РЕАЛЬНЫЕ
  // поисковые интенты, живые формулировки запросов, вопросы People-Also-Ask /
  // AI Overviews и смежные семантические темы — чтобы контент-стратег строил
  // план тем от интентов пользователя и покрывал как классический поиск, так и
  // ответы ИИ-выдачи.
  perplexityTopicResearcher: `ROLE: Senior Search Intent Researcher & SEO Demand Analyst.
MISSION: Через поиск в интернете собрать РЕАЛЬНЫЙ спрос и поисковые интенты пользователей по нише: "{{input_target_service}}" (регион: {{input_region}}). Данные нужны, чтобы построить контент-план, который ранжируется в классическом поиске И попадает в ответы ИИ-выдачи (AI Overviews, ИИ-ассистенты).
INSTRUCTIONS:
1. Найди реальные поисковые запросы и живые формулировки, которые пользователи вводят по этой нише (не выдумывай — опирайся на автодополнение, форумы, отзывы, People-Also-Ask).
2. Для каждого запроса определи первичный интент (informational/commercial/transactional/navigational), фасет (how-to/comparison/definition/troubleshooting/reviews/alternatives/pricing-research/case-studies/…) и стадию воронки (TOFU/MOFU/BOFU).
3. Собери вопросы формата People-Also-Ask и вопросы, на которые отвечают ИИ-ассистенты и AI Overviews (то, что реально триггерит ИИ-ответ).
4. Предложи СМЕЖНЫЕ темы и семантические кластеры для расширения охвата интентов (соседние подтемы, которые пользователи ищут «до» и «после» основной темы).
5. Собери ключевые сущности / LSI (имена продуктов, методик, регуляторики, инструментов), релевантные нише.
6. Найди 3-5 актуальных фактов/цифр и последние тренды, обосновывающие «почему сейчас».
RULES: Строго опирайся на интенты пользователей и реальные данные поиска. Не придумывай запросы, которых нет. Если по пункту данных нет — верни пустой массив.
OUTPUT FORMAT: Верни СТРОГО валидный JSON без markdown-обёрток:
{
  "user_intents": [{"query": "string", "intent": "informational|commercial|transactional|navigational", "facet": "string", "stage": "TOFU|MOFU|BOFU"}],
  "adjacent_topics": [{"topic": "string", "why": "string", "semantic_cluster": "string"}],
  "paa_questions": ["string"],
  "ai_overview_questions": ["string"],
