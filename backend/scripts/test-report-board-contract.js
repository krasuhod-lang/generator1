...
const projectsPage = read('frontend/src/views/ProjectDetailPage.vue');
const projectsController = read('backend/src/controllers/projects.controller.js');
const projectsRoutes = read('backend/src/routes/projects.routes.js');
const metrikaService = read('backend/src/services/projects/metrikaService.js');
const integrationVault = read('backend/src/services/integrations/integrationVault.js');
const keyProbe = read('backend/src/services/integrations/integrationKeyProbe.js');
...
assert(publicPage.includes('router.replace'), 'public page must persist tab/range state in URL');
assert(publicPage.includes('public-project-header'), 'public page must render project header above toolbar');
assert(!publicPage.includes('REPORT BOARD'), 'public page must not expose internal REPORT BOARD label');
assert(publicPage.includes("id: 'metrika'"), 'public page must expose conditional Metrika tab');
assert(publicPage.includes('show-header="false"'), 'public page must avoid duplicate renderer header');
assert(renderer.includes('traffic-summary-strip'), 'ReportRenderer must show source metric totals below charts');
assert(renderer.includes('id="report-metrika"'), 'ReportRenderer must render Metrika section');
assert(renderer.includes('pageSourceLabel'), 'ReportRenderer must distinguish Webmaster queries from URL pages');
assert(aggregator.includes("wrap('metrika'"), 'dataAggregator must fetch Metrika as an optional cached section');
assert(aggregator.includes('page_sources'), 'dataAggregator must expose source semantics for page rows');
assert(projectsController.includes('yandex_metrika_counter_id'), 'project controller must persist Metrika counter ID');
