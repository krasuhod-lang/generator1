...
const checks = [
  ...
  ['new analysis triggers immediate outbox dispatch', () => assert.match(controller, /publishPendingOutbox\(db, 20\)/)],
  ['queued analyses reconcile against BullMQ state', () => assert.match(reliability, /projectAnalysisQueue\.getJob\(safeJobId\)/)],
  ['published outbox is reset when queued job is missing', () => assert.match(reliability, /published_at=NULL/) && assert.match(reliability, /project_analysis_job_missing_or_terminal/)],
