...
test('коммерческие разделы определяются по сегменту пути', () => {
  const commUrls = [
    ...
    'https://site.com/product/iphone-15/',
    ...
    // WooCommerce/CMS-маркеры: product-category, product-tag и алиасы
    'https://site.com/product-category/okna/',
    'https://site.com/product_category/okna/',
    'https://site.com/product-cat/okna/',
    'https://site.com/product_cat/okna/',
    'https://site.com/product-tag/aluminum/',
    'https://site.com/product_tag/aluminum/',
  ];
...
test('страница без маркеров — интент не определён (unknown)', () => {
  // ТЗ-правка: страницы вида /domain/название (без catalog/blog/...) могут
  // быть и коммерческими, и информационными. Помечаем 'unknown', чтобы они
  // попали в оба списка отчёта с подписью «не удалось распознать».
...
test('главная страница без сегментов — unknown', () => {
...
test('сегмент матчится целиком, без ложных срабатываний на подстроке', () => {
  ...
  const r1 = classifyUrl('https://site.com/particles/x');
  assert.strictEqual(r1.confident, false);
  assert.strictEqual(r1.intent, UNKNOWN);
  const r2 = classifyUrl('https://site.com/storefront/x');
  assert.strictEqual(r2.confident, false);
  assert.strictEqual(r2.intent, UNKNOWN);
});
...
test('isInformationalUrl возвращает булево', () => {
  ...
  // unknown — не информационная
  assert.strictEqual(isInformationalUrl('https://site.com/plastikovye-okna/'), false);
});

test('пустой/невалидный URL не падает и считается unknown', () => {
  assert.strictEqual(classifyUrl('').intent, UNKNOWN);
  assert.strictEqual(classifyUrl(null).intent, UNKNOWN);
  assert.strictEqual(classifyUrl('not a url').intent, UNKNOWN);
});
