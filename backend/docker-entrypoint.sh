...
if [ "$PROXY_FOUND" -eq 0 ]; then
  echo ""
  echo "  ❌ ОШИБКА: Ни одна переменная GEMINI_PROXY_* не задана!"
  echo ""
  echo "  Добавьте в .env файл (в корне проекта, рядом с docker-compose.yml):"
  echo ""
  ...
  echo "    GEMINI_PROXY_HOST=155.212.59.188"
  echo "    GEMINI_PROXY_PORT=64464"
